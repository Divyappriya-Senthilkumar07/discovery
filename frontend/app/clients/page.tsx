    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Client DNA Profiles</h1>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center p-12 text-center">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Loading Client DNA profiles...</p>
        </div>
      ) : clients.length === 0 ? (
        <div className="glass-panel rounded-2xl p-12 text-center border border-slate-200 dark:border-slate-800">
          ...
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">No Client Profiles Configured</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {clients.map((client) => {
            ...
            return (
              <div
                key={client.id}
                className="glass-card rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800/80 hover:border-indigo-500/40 transition flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">{client.name}</h2>
                      {client.parent_company && (
                        <p className="text-xs text-slate-600 dark:text-slate-400">
                          Parent: <span className="text-slate-800 dark:text-slate-200 font-medium">{client.parent_company}</span>
                        </p>
                      )}
                    </div>
                    {hasOverrides && (
                      <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-500/20 flex items-center gap-1">
                  ...
                  {/* Industry Terms */}
                  <div className="mb-4">
                    <span className="text-[10px] uppercase font-semibold text-indigo-600 dark:text-indigo-400 tracking-wider block mb-1.5">
                      Domain Jargon
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {client.industry_terms?.slice(0, 5).map((term, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-md text-[10px] bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-medium"
                        >
                          {term}
                        </span>
                      ))}
                  ...
                  {/* Aliases & Subs */}
                  <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800/80 pt-3">
                    <p className="truncate">
                      <strong className="text-slate-800 dark:text-slate-300">Aliases:</strong> {client.aliases?.join(', ') || 'None'}
                    </p>
                    <p className="truncate">
                      <strong className="text-slate-800 dark:text-slate-300">Subsidiaries:</strong>{' '}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-lg glass-panel rounded-2xl p-6 border border-slate-200 dark:border-slate-700/80 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Generate Client DNA Profile</h3>
              </div>
            ...
            <form onSubmit={handleCreateClient} className="space-y-4 mt-4">
              <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Company / Brand Name</label>
                <input
                  type="text"
                  required
                  value={clientName}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Seed Description (Optional)</label>
                <textarea
                  rows={3}
                  ...
                  className="w-full px-3 py-2 text-xs rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none shadow-sm"
                />
              </div>

              <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Corporate URL (Optional)</label>
                <input
                  type="url"
                  value={seedUrl}
