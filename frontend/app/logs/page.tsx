      ...
      {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2.5 rounded-xl bg-gradient-to-tr from-violet-500/20 to-purple-500/20 border border-violet-500/30 text-violet-600 dark:text-violet-400">
                <Activity className="w-6 h-6" />
              </div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Agent Audit Log & Explainability</h1>
            </div>
          ...
          >
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin text-violet-600 dark:text-violet-400' : ''}`} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Total Agent Decisions</span>
            ...
            <div className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Average Decision Latency</span>
                <Clock className="w-4 h-4 text-cyan-600 dark:text-cyan-400" />
              </div>
              <div className="text-3xl font-black text-cyan-600 dark:text-cyan-400 mt-2 font-mono">
                {stats.average_decision_latency_ms} <span className="text-lg font-normal text-slate-500 dark:text-slate-400">ms</span>
              </div>
            ...
            <div className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Autonomous Agents</span>
                <Database className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              </div>
              <div className="text-3xl font-black text-emerald-600 dark:text-emerald-400 mt-2 font-mono">8 / 8</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2.5 rounded-xl bg-white dark:bg-slate-950/80 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:border-violet-500 shadow-sm"
              />
            </div>
            ...
            <div>
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl bg-white dark:bg-slate-950/80 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:border-violet-500 shadow-sm"
              >
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">Min Conf:</span>
              <input
                type="range"
                min="0"
                max="0.9"
                ...
                className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer accent-violet-500"
              />
        ...
        {loading ? (
          <div className="py-20 text-center space-y-3">
            <RefreshCw className="w-8 h-8 animate-spin text-violet-600 dark:text-violet-400 mx-auto" />
            <p className="text-xs text-slate-500 dark:text-slate-400">Streaming audit records from database...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="glass-card p-12 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-3 shadow-sm">
            <Activity className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto" />
          ...
          <div className="space-y-3">
            {logs.map(log => {
              ...
              return (
                <div
                  key={log.id}
                  className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700/80 transition-all overflow-hidden shadow-sm"
                >
                  <div
                    onClick={() => setExpandedLogId(isExpanded ? null : log.id)}
                    className="p-5 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-4 select-none hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors"
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[10px] font-mono text-cyan-700 dark:text-cyan-400 flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-3 text-slate-500 text-xs font-mono">
                      ...
                      <div className="p-1 rounded bg-slate-100 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400">
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 pt-2 border-t border-slate-200 dark:border-slate-800/60 bg-slate-50/60 dark:bg-slate-950/50 space-y-3 animate-in fade-in duration-200">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                        <div>
                          <span className="font-semibold text-slate-600 dark:text-slate-400 block mb-1">Input Summary:</span>
                          <div className="p-3 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-300 font-mono text-[11px] overflow-x-auto">
                            {log.input_summary || 'N/A'}
                          </div>
                        </div>

                        <div>
                          <span className="font-semibold text-slate-600 dark:text-slate-400 block mb-1">Output Summary:</span>
                          <div className="p-3 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-300 font-mono text-[11px] overflow-x-auto">
                      ...
                      {log.metadata && Object.keys(log.metadata).length > 0 && (
                        <div>
                          <span className="font-semibold text-slate-600 dark:text-slate-400 block mb-1 flex items-center gap-1.5">
                            <Code className="w-3.5 h-3.5 text-violet-600 dark:text-violet-400" />
                            <span>Raw Decision Metadata:</span>
                          </span>
                          <pre className="p-3 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 text-violet-700 dark:text-violet-300 font-mono text-[11px] overflow-x-auto">
