...
export default function DailyBriefPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Executive Daily Brief</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select
            ...
            className="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-white text-sm font-medium focus:outline-none focus:border-emerald-500/50 transition-all shadow-sm"
          >
            {clients.map(c => (
      ...
      {/* Status Alert */}
      {statusMessage && (
        <div className="p-4 rounded-xl bg-emerald-50 dark:bg-slate-900/90 border border-emerald-200 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-300 text-sm flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            <span>{statusMessage}</span>
          </div>
          <button onClick={() => setStatusMessage(null)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white text-xs">
            Dismiss
          </button>
        </div>
      )}
      ...
      {selectedClient && (
        <div className="glass-card p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 font-mono">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Aggregating story clusters and clustering vectors...</p>
        </div>
      ) : briefs.length === 0 ? (
        <div className="glass-card p-12 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-4">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Intelligence Briefs Generated Yet</h3>
        ...
        <div className="space-y-6">
          {briefs.map((brief, idx) => {
            ...
            return (
              <div
                key={brief.id}
                className="glass-card rounded-2xl border border-slate-200/80 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700/80 transition-all overflow-hidden"
              >
                {/* Top Bar: Cluster Metadata */}
                <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    <span className="w-6 h-6 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 font-mono text-xs font-bold flex items-center justify-center">
                      #{idx + 1}
                    </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium flex items-center gap-1.5 transition-colors"
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2.5 flex items-center gap-1.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {brief.contributing_sources.map((domain, sIdx) => (
                        <span
                          key={sIdx}
                          className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 text-xs font-mono text-slate-700 dark:text-slate-300 shadow-sm"
                        >
                    ...
                    <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 space-y-2">
                      ...
                      <div className="space-y-1.5 pt-1">
                        {brief.outlier_articles.map((outlier, oIdx) => (
                          <div
                            key={oIdx}
                            className="text-xs text-amber-900 dark:text-amber-200/90 font-medium pl-3 border-l-2 border-amber-500"
                          >
                            {outlier}
                          </div>
                        ))}
