      ...
      {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-bold text-white tracking-tight">Source Intelligence & Trust</h1>
          ...
          >
            <RefreshCw className={`w-4 h-4 ${loadingDiscovery || loadingCredibility ? 'animate-spin text-cyan-400' : ''}`} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="glass-card p-5 rounded-2xl border border-slate-800">
            <div className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">Indexed Media Domains</div>
            <div className="text-2xl font-black text-white">{credibilityList.length}</div>
            <div className="text-xs text-slate-500 mt-1">Total vetted publishers</div>
          </div>
          <div className="glass-card p-5 rounded-2xl border border-slate-800">
            <div className="text-xs font-semibold uppercase tracking-wider text-indigo-400 mb-1">Tier 1 Global Leaders</div>
            <div className="text-2xl font-black text-indigo-400">{tier1Count}</div>
            <div className="text-xs text-slate-500 mt-1">High-impact national press</div>
          </div>
          <div className="glass-card p-5 rounded-2xl border border-slate-800">
            <div className="text-xs font-semibold uppercase tracking-wider text-amber-400 mb-1">Pending Discovery Review</div>
          ...
          <div className="glass-card p-5 rounded-2xl border border-slate-800">
            <div className="text-xs font-semibold uppercase tracking-wider text-cyan-400 mb-1">Analyst Overrides</div>
            <div className="text-2xl font-black text-cyan-400">{overriddenCount}</div>
        ...
        {/* Status Notification */}
        {statusMessage && (
          <div className="p-4 rounded-xl bg-slate-900/90 border border-cyan-500/30 text-cyan-300 text-sm flex items-center justify-between">
            <div className="flex items-center gap-2">
        ...
        {activeTab === 'discovery' && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Review Status:</span>
            ...
            {loadingDiscovery ? (
              <div className="py-20 text-center space-y-3">
                ...
                <p className="text-xs text-slate-400">Loading discovered candidate sources...</p>
              </div>
              ...
              <div className="glass-card p-12 rounded-2xl border border-slate-800 text-center space-y-3">
                ...
                <h4 className="text-base font-bold text-white">No Sources in &ldquo;{discoveryFilter}&rdquo;</h4>
              ...
              <div className="space-y-4">
                {filteredDiscovered.map(source => {
                  ...
                  return (
                    <div
                      key={source.id}
                      className="glass-card p-5 rounded-2xl border border-slate-800 hover:border-slate-700/80 transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-5"
                    >
                      <div className="space-y-2 flex-1">
                        <div className="flex flex-wrap items-center gap-2.5">
                          <span className="text-base font-bold text-white font-mono">{source.domain}</span>
                          ...
                          <span className="px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-[11px] text-cyan-400">
                            Triggered for: <strong className="font-semibold">{clientName}</strong>
                          </span>
                        </div>

                        <p className="text-xs text-slate-300 leading-relaxed">
                          <strong className="text-slate-400">Agent Reason:</strong> {source.reason}
                        </p>
                        ...
                        {source.status === 'pending_review' ? (
                          <>
                            <button
                              ...
                              disabled={isActing}
                              className="px-4 py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-semibold text-xs flex items-center gap-1.5 transition-all disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              disabled={isActing}
                              className="px-4 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 font-semibold text-xs flex items-center gap-1.5 transition-all disabled:opacity-50"
                            >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-white text-xs focus:outline-none focus:border-cyan-500"
                />
              </div>
                      ...
                      <tr key={idx} className="hover:bg-slate-900/40 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          {item.manually_overridden ? (
                            ...
                            <span className="text-[11px] text-slate-500">Algorithmic</span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium text-xs transition-colors inline-flex items-center gap-1.5"
                          >
        ...
        {selectedDomainForOverride && (
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
            <div className="glass-card max-w-md w-full p-6 rounded-3xl border border-slate-700 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-cyan-400" />
                  <h3 className="text-base font-bold text-white">Override Domain Credibility</h3>
                </div>
                <button
              ...
              <div>
                <div className="flex justify-between text-xs mb-2">
                  <span className="text-slate-400 uppercase tracking-wider font-semibold">Trust Score</span>
                  <span className="font-mono font-bold text-cyan-400">{(overrideScore * 100).toFixed(0)}%</span>
                </div>
                <input
                  type="range"
                  min="0.10"
